...
public class BidDAO<T> extends AbstractDAO<T> {
	...
	private static final String SELECT_QUERY = "SELECT x FROM Bid x ORDER BY x.id";
		...
		try {
			...
			if (log.isDebugEnabled()) {
				if (el != null) {
					...
					log.error("Bid with Id=" + param + " not found.");
			}
		} catch (Exception e) {
			log.error("An error occurred while searching for the bid with Id="
					+ param, e);

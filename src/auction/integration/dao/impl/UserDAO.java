...
public class UserDAO<T> extends AbstractDAO<T> {
	...
	protected String createSelectQuery() {
		return "SELECT x FROM User x ORDER BY x.id";
	}
	...
	public T find(Object param) {
		...
		try {
			...
			if (log.isDebugEnabled()){
				...
				} else log.error("User with Id=" + param + " not found.");
			}			
		} catch (Exception e) {
	...
	public User findByLogin(String login){
		...
		try {
			...
			if (log.isDebugEnabled()){
				if (list.size() != 0) {
					log.debug("User with login=" + login + " successfully found."); 
				} else log.error("User with login=" + login + " not found.");
			}
		} catch (Exception e) {
